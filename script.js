...
function updateStartButton() {
  ...
  // جعل وضع الشاشة الكاملة اختياريًا لتجنب منع المستخدمين على الأجهزة غير المدعومة
  const requirementsMet = musicEnabled;

  // تحديث النص ليشير فقط للموسيقى إذا كان الشاشة الكاملة غير مدعومة
  if (!requirementsMet) {
...
function triggerCakeReveal() {
  ...
  if (cakeContainer) {
    ...
    cakeContainer.addEventListener('click', function () {
      ...
      const candle = document.querySelector('.cake-candle');
      if (candle) {
        candle.style.transition = 'opacity 1s';
